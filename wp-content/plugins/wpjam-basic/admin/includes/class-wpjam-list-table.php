...
class WPJAM_List_Table extends WP_List_Table {
	...
	public function parse_args($args){
		...
		if($args['actions']){
			foreach ($args['actions'] as $action_key => $action) {
				...
				$capability	= $action['bulk_capability'] ?? ($action['capability'] ?? $args['bulk_capability']);
		...
		$args['options_columns']	= $args['options_columns'] ?? [];
	...
	public function set_summary($summary){
		if($summary && empty($this->_args['summary'])){
			$this->_args['summary']	= $summary; 
		}
	}

	public function get_summary(){
		return $this->_args['summary']	?? '';
	}
	...
	protected function parse_item($raw_item){
		...
		$item = apply_filters(wpjam_get_filter_name($this->_args['singular'], 'item'), $item, $raw_item);
	...
	protected function column_callback($column_name, $id, $data_type='form'){
		...
		if(empty($columns) || !isset($columns[$column_name]) || empty($fields) || !isset($fields[$column_name])){
			return null;
		}
		...
		if(isset($field['column_callback']) || empty($model)){
			return wpjam_column_callback($column_name, array(
				'id'		=> $id,
				'field'		=> $field,
				'data_type'	=> $data_type
			));
		}elseif($model && method_exists($model, 'column_callback')){
	...
	public function list_page(){
		wpjam_admin_plugin_page_title($this->_args['title'], $this->get_subtitle(), $this->get_summary());
		...
		if(method_exists($model, 'list_page')){
			_deprecated_function($model.'::list_page', 'WPJAM 3.9', '请使用'.$model.'::admin_footer代替');
			$model::list_page();
		}
	...
	public function column_cb($item){
		if($primary_key = $this->_args['primary_key']){
			if(current_user_can($this->_args['capability'], $item[$primary_key])){
	...
	public function get_current_action_js_args(){
		...
		$data	= $data ? wp_parse_args(urldecode($data)) : [];
	...
	public function _js_vars() {
		...
		jQuery(function($){
			...
			<?php if($args = $this->get_current_action_js_args()){ 
