...
function wpjam_line_chart($counts_array, $labels, $args=array(), $type = 'Line'){
	...
	<table class="wp-list-table widefat striped" cellspacing="0">
		<thead>
			<tr>
				<th scope="col" id="day" class="manage-column column-day column-primary"><?php echo $day_label;?></th>
				<?php foreach ($labels as $morris_ykey => $morris_label) { $totol_array[$morris_ykey] = 0;?>
				<th  scope="col" id="<?php echo $morris_ykey; ?>" class="manage-column column-<?php echo $morris_ykey;?>"><?php echo $morris_label;?></th>	
				<?php } ?>
			</tr>
		</thead>
		<tbody>
			...
			if($show_link){
				$day_label	= ($day_labels && isset($day_labels[$day]))?$day_labels[$day]:$day;
				$day	= '<a href="'.$total_link.'&'.$day_key.'='.$day.'">'.$day_label.'</a>';
			}else{
				$day	= ($day_labels && isset($day_labels[$day]))?$day_labels[$day]:$day;
			}
			?>
			<tr>
				<td class="day column-day column-primary" data-colname="<?php echo $day_label; ?>">
					<?php echo $day; ?>
					<button type="button" class="toggle-row"><span class="screen-reader-text">显示详情</span></button>		
				</td>
				<?php foreach($labels as $morris_ykey => $morris_label){ $count = ($counts[$morris_ykey])??0; 
					if(strpos($labels[$morris_ykey], '%')===false && strpos($labels[$morris_ykey], '#')===false){
						$totol_array[$morris_ykey] += $count;
					}?>
				<td class="day column-<?php echo $morris_ykey; ?>" data-colname="<?php echo $morris_label; ?>"><?php echo $count; ?></td>
				<?php } ?>
			</tr>
			<?php } ?>
			...
			<tr>
				...
				<?php foreach($labels as $morris_ykey => $morris_label){ ?>
				<td class="day column-<?php echo $morris_ykey; ?>" data-colname="<?php echo $morris_label; ?>">
					<?php if(isset($counts[$morris_ykey]) && strpos($labels[$morris_ykey], '%')===false && strpos($labels[$morris_ykey], '#')===false){
...
function wpjam_donut_chart($counts, $args=array()) {
	...
	<div style="display:table; margin-bottom:20px;">
		...
		<div style="display: table-row;">
			...
			<div style="display: <?php echo $table_width ? 'table-cell' : 'none'; ?>; float:left; width:<?php echo $table_width; ?>px; margin-right:20px;">
				<table class="wp-list-table widefat striped" cellspacing="0">
					<thead>
						<tr>
					...
					<?php foreach ($counts as $count) { $alternate = empty($alternate)?'alternate':''; $i++; $count = (array)$count;?>
						...
						if($show_link){
							$value	= $count[$key]??$label;
							$link	= $total_link.'&'.$key.'='.$value;
						}
