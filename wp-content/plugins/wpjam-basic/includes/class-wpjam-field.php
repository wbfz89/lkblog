...
class WPJAM_Field{
	...
	public static function column_callback($column_name, $args=[]){
		...
		if($data_type == 'form'){
			...
			if($item_value && isset($item_value[$column_name])){
				$column_value	= $item_value[$column_name];
			}
		}elseif($data_type == 'post_meta'){
			if(metadata_exists('post', $id, $column_name)){
				$column_value	= get_post_meta($id, $column_name, true);	
			}
		}elseif($data_type == 'term_meta'){
			if(metadata_exists('term', $id, $column_name)){
				$column_value = get_term_meta($id, $column_name, true);
			}
		}elseif($data_type == 'user_meta'){
			if(metadata_exists('user', $id, $column_name)){
				$column_value = get_user_meta($id, $column_name, true);
			}
		}
