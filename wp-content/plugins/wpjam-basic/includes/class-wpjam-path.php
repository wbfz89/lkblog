...
class WPJAM_Path {
	...
	public function get_fields($path_type){
		if($this->fields){
			return $this->fields[$path_type] ?? '';
		}else{
			$fields	= [];
	...
	public function get_path($type, $args=[]){
		...
		if($type == 'template'){
			...
			if($callback){
				...
				if($this->page_type == 'post_type'){
					return $this->get_post_type_path($args);
				}elseif($this->page_type == 'taxonomy'){
					return $this->get_taxonomy_path($args);
				}else{
	...
	public function get_post_type_path($args){
	...
	public function get_taxonomy_path($args){
	...
	public static function create($page_key, $args=[]){
		...
		if(!empty($args['path_type'])){
			...
			if(!empty($args['fields'])){
				$path_obj->set_fields($args['path_type'], $args['fields']);
			}
